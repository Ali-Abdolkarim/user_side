...
class _HomePageState extends State<HomePage> {
  ...
  void enterExam() async {
    ...
    if (response.docs.isEmpty) {
      ...
      var currentTest = response.docs[0].data();
      if (DateTime.now().millisecondsSinceEpoch < currentTest[Texts.DATE]) {
        Get.defaultDialog(
            radius: 6,
            cancel: SimpleButton(
              'Cancel',
      ...
      //do the checking here,, start and finish is not correct
      DateTime timeAndDuration = DateTime.fromMillisecondsSinceEpoch(
          currentTest[Texts.DATE] + currentTest[Texts.DURATION] * 60 * 1000);
      ...
      var examTakenResponse = await db
          ...
          .where(Texts.EXAM_ID, isEqualTo: response.docs[0].id)
          .get();
      ...
      Get.defaultDialog(
          ...
          confirm: SimpleButton(
            ...
            action: () async {
              var temp = {
                Texts.EXAM_ID: response.docs[0].id,
